...
int rodaOpenmp (ArgumentosMandel args){

    int **matriz = malloc(args.altura * sizeof(int *));
    if (matriz == NULL){
        fprintf (stderr, "Falha ao alocar memoria no openmp\n");
        return -1;
    }
    ...
    omp_set_num_threads(args.numeroThreads);
    ...
    sprintf(nomeArquivo, "mandelbrot_%s_openmp.pgm", args.login);

    if (escreverPgm(nomeArquivo, matriz, args.largura, args.altura) != 0){
        fprintf(stderr, "Falha em escrever no arquivo pgm openmp\n");
        return -1;
    }

    FILE *arquivoTempo = fopen ("times.txt", "a");
    if (arquivoTempo == NULL){
        fprintf(stderr, "Erro ao abrir o times.txt no openmp\n");
        return -1;
    }

    fprintf(arquivoTempo, "Openmp: %fs\n", duracao);
    fclose(arquivoTempo);
